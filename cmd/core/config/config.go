...
func GetCoreConfig() (*general.SectionService, error) {
	...
	data := &general.SectionService{
		...
		S3CredentialKey: general.S3CredentialKey{
			AccessKeyID:     viper.GetString("S3CREDENTIALKEY.ACCESS_KEY_ID"),
			SecretAccessKey: viper.GetString("S3CREDENTIALKEY.SECRET_ACCESS_KEY"),
			Region:          viper.GetString("S3CREDENTIALKEY.REGION"),
			BucketName:      viper.GetString("S3CREDENTIALKEY.BUCKET_NAME"),
			Endpoint:        viper.GetString("S3CREDENTIALKEY.ENDPOINT"),
		},
	}
