...
func GenerateJWTUserID(session string, userid int64) (string, string, error) {
	//Create Access Token
	accessToken, err := generateAccessTokenUserID(session, userid)
	if err != nil {
		return "", "", err
	}
...
func generateAccessTokenUserID(session string, userid int64) (string, error) {
	accessClaims := Claims{
		StandardClaims: jwt.StandardClaims{
			Issuer:    issuer,
			ExpiresAt: time.Now().UTC().Add(jwtCfg.atd).Unix(),
